...
type SourcedFigureMetric = {
  captionText: string;
  hasCaption: boolean;
  hasSourceLink: boolean;
  imageMetrics: SourcedImageMetric[];
};
...
async function expectAccessibleSourcedMedia(page: Page): Promise<void> {
  ...
  const figures = await page.evaluate<SourcedFigureMetric[]>(() =>
  ...
  for (const figure of figures) {
    expect(figure.hasCaption).toBe(true);
    expect(figure.hasSourceLink).toBe(true);
    expect(figure.captionText).toMatch(
      /source|creator|license|government|public domain|reuse|rights/i,
    );

    for (const image of figure.imageMetrics) {
...
test('representative routes render accessible sourced media without page overflow', async ({
  page,
}) => {
  for (const viewport of responsiveViewports) {
    await page.setViewportSize(viewport);
    ...
    for (const route of visualCoverageRoutes) {
      ...
      await expectAccessibleSourcedMedia(page);
      await expectNoPageHorizontalOverflow(page);
    }
  }
});

test('all generated routes avoid page overflow and broken sourced media', async ({
  page,
}, testInfo) => {
...
test('representative visual audit pages have relevant labels, captions, and no overflow', async ({
  ...
  const auditedTechnologies = [
    {
      ...
      alt: /Cutaway diesel vehicle/i,
      source: /Diesel vehicle component cutaway/i,
    },
    {
    ...
    },
    {
      ...
      alt: /Cutaway all-electric sedan/i,
      source: /All-electric vehicle component cutaway/i,
    },
    {
      route: '/technologies/anti-lock-braking-system/',
      screenshot: 'audit-tech-abs',
      alt: /Cutaway gasoline car/i,
      source: /Gasoline vehicle component cutaway/i,
    },
    {
  ...
  for (const viewport of responsiveViewports) {
    ...
    await expect(
      page.getByRole('link', { name: /Turbocharger/i }).first(),
    ).toBeVisible();
    await captureLayoutScreenshot(
      page,
      testInfo,
    ...
    for (const technology of auditedTechnologies) {
      ...
      const firstFigure = page.locator('figure.sourced-media').first();
      await expect(firstFigure.locator('img')).toHaveAttribute(
        'alt',
        technology.alt,
      );
  ...
  await expect(page.getByText(/Benespit/i).first()).toBeVisible();
